...
def _weblearns_post_init_hook(cr, registry):
    print("Hello you just executes post init hook method.")
    cr.execute("update res_partner set vat='1234567891' where vat is null")
    cr.commit()

def _weblearns_uninstall_hook(cr, registry):
    print("Hello your module uninstalled from uninstall_hook")
    cr.execute("update res_partner set vat='' where vat='1234567891' ")
    cr.commit()
